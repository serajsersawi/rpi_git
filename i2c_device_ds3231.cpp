...
void i2c_device_ds3231::enableAlarm(ALARM_NO enableAlarmNo){
	...
	switch (enableAlarmNo){
		...
		default: cerr << "Invalid alarm number!" << endl;  break;
...
void i2c_device_ds3231::start32kHz(){
	...
	unsigned int RegisterVal = this->readRegister(CTRL_STAT_REG);
	this->writeRegister(CTRL_STAT_REG, (RegisterVal | (1 << 3))); 

	
}

void i2c_device_ds3231::stop32kHz(){
...
unsigned int i2c_device_ds3231::setSeconds(unsigned int seconds, unsigned int reg){
	...
	if(targetRegister != 13){
		if(seconds < 60){
			this->writeRegister(targetRegister, (oldRegisterVal | (decimalToBCD(seconds) & 0x7F)));
			//update the object 
			this->seconds = getSeconds();
			return 0;
		}
...
unsigned int i2c_device_ds3231::setMinutes(unsigned int minutes, unsigned int reg){
	cout << "minutes input" << minutes << endl;
	...
	unsigned int oldRegisterVal = this->readRegister(targetRegister);
	if(minutes < 60){
		this->writeRegister(targetRegister, (oldRegisterVal | (decimalToBCD(minutes) & 0x7F)));
		this->minutes = getMinutes();
		return 0;
	}
	else{
